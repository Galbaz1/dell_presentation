...
const Slide7: React.FC = () => {
  ...
  const [autoHoverIndex, setAutoHoverIndex] = useState<number>(0);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setAutoHoverIndex((prevIndex) => {
        ...
        return prevIndex >= blocks.length - 1 ? 0 : prevIndex + 1;
      });
    }, 10000); // 10 seconds
    ...
    // Update hoveredBlock whenever autoHoverIndex changes
    const hoverEffect = () => {
      setHoveredBlock(autoHoverIndex);
    };
    hoverEffect();
  ...
  }, [autoHoverIndex]); // Depend on autoHoverIndex
      ...
      >
        {blocks.map((block, index) => (
          <motion.div
            ...
            custom={index}
            animate={hoveredBlock === index ? "hover" : "initial"}
          >
            <div
