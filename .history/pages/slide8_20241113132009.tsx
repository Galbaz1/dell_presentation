...
""")`;
  ...
  useEffect(() => {
    ...
    const startScroll = async () => {
      if (scrollRef.current) {
        ...
        // Update the animation based on direction and speed
  ...
  }, [controls, scrollDirection, scrollSpeed, isKeyHeld]);
  ...
  useEffect(() => {
      ...
      if (event.repeat) {
        ...
        switch (event.key) {
          case 'ArrowLeft':
            ...
            setScrollSpeed(5); // Faster scroll speed when key held
            break;
          case 'ArrowRight':
    ...
    const handleKeyUp = (event: KeyboardEvent) => {
      if (event.key === 'ArrowLeft' || event.key === 'ArrowRight') {
        setIsKeyHeld(false);
        setScrollSpeed(30); // Reset to default speed
        setScrollDirection(1); // Reset to default direction
      }
    };
