    ...
    <div className="bg-gradient-to-br from-emerald-50 to-emerald-100 p-8 rounded-xl shadow-lg min-h-screen overflow-hidden relative">
      ...
      >
        {solutions.map((solution, index) => (
          <motion.div
            key={solution.title}
            className="bg-white p-6 rounded-lg shadow-lg cursor-pointer transform transition-all duration-300 relative"
            style={{ borderWidth: 2, borderStyle: 'solid', borderColor: solution.color.border }}
          ...
          >
            {/* Glow effect */}
            <motion.div
              className="absolute inset-0 rounded-lg filter blur-xl"
            ...
            <ul className="space-y-3">
              {solution.items.map((item, i) => (
                ...
                >
                  <Sparkles className="w-4 h-4 mr-2 flex-shrink-0" style={{ color: solution.color.glow }} />
                  {item}
                </motion.li>
              ))}
            </ul>
