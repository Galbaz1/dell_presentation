...
const Slide7: React.FC = () => {
  ...
  const [autoHover, setAutoHover] = useState<boolean>(true);
  ...
  useEffect(() => {
    ...
    let currentBlock = -1;
    const hoverDuration = 10000; // 10 seconds
    ...
    // Initial hover on first block
    setHoveredBlock(0);

    return () => {
      clearInterval(timer);
    };
  }, [autoHover]); // Only run once when component mounts and when autoHover changes
  ...
  const blocks = [
    ...
    },
    {
      ...
      title: "Validation",
      description: "Pydantic is all you need",
      color: {
        bg: "#fff1f2",
        border: "#f43f5e",
        text: "#881337"
      ...
      >
        {blocks.map((block, index) => (
          <motion.div
            ...
            custom={index}
            whileHover={autoHover ? {} : "hover"}
            onHoverStart={() => !autoHover && setHoveredBlock(index)}
            onHoverEnd={() => !autoHover && setHoveredBlock(null)}
          >
            <div
