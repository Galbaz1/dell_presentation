...
const FootstepsAnimation: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // New vertical path starting from where horizontal path ends
    const verticalPath = [
    ...
    const animateFootstep = (
      ...
      delay: number,
      isLeft: boolean,
      startRotation: number = 90 // Default rotation for horizontal movement
    ): void => {
      path.forEach((point, index) => {
        ...
        const verticalOffset = isLeft ? 5 : -5;
        const horizontalOffset = isLeft ? -12 : 12;
        
        gsap.to(foot, {
          x: point.x + (path === horizontalPath ? horizontalOffset : 0),
          y: point.y + (path === horizontalPath ? verticalOffset : horizontalOffset),
          rotation: startRotation + rotation + (isLeft ? -15 : 15),
          duration: 0.6,
          delay: delay + (index * 0.7),
          ease: "power2.inOut",
          onComplete: () => createGhostTrail(
            foot, 
            point.x + (path === horizontalPath ? horizontalOffset : 0),
            point.y + (path === horizontalPath ? verticalOffset : horizontalOffset),
            startRotation + rotation + (isLeft ? -15 : 15)
          ),
        });
      });
    };
    ...
    // First animate horizontal path
    animateFootstep(leftFootRef.current, horizontalPath, 0, true, 90);
    animateFootstep(rightFootRef.current, horizontalPath, 0.35, false, 90);

    // Then animate vertical path with 180-degree rotation
    const horizontalDuration = horizontalPath.length * 0.7;
    animateFootstep(leftFootRef.current, verticalPath, horizontalDuration, true, 180);
    animateFootstep(rightFootRef.current, verticalPath, horizontalDuration + 0.35, false, 180);
