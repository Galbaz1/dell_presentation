...
const FootstepsAnimation: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // New vertical path with exact same increments as horizontal
    const verticalPath = [
      { x: 1520, y: 100 },    // Start where horizontal path ended
      { x: 1525, y: 162 },    // Wave right (62px increment to match horizontal)
      { x: 1530, y: 224 },    // Peak right
      { x: 1525, y: 286 },    // Wave back center
      { x: 1520, y: 348 },    // Center
      { x: 1525, y: 410 },    // Wave right
      { x: 1530, y: 472 },    // Peak right
      { x: 1525, y: 534 },    // Wave back center
      { x: 1520, y: 596 },    // Center
      { x: 1525, y: 658 },    // Wave right
      { x: 1520, y: 720 },    // End position
      { x: 1525, y: 142 },    // Wave right (42px increment like horizontal)
      { x: 1530, y: 184 },    // Peak right
      { x: 1525, y: 226 },    // Wave back center
      { x: 1520, y: 268 },    // Center
      { x: 1525, y: 310 },    // Wave right
      { x: 1530, y: 352 },    // Peak right
      { x: 1525, y: 394 },    // Wave back center
      { x: 1520, y: 436 },    // Center
      { x: 1525, y: 478 },    // Wave right
      { x: 1520, y: 520 },    // End position
    ];
    ...
    const animateFootstep = (
      ...
      path.forEach((point, index) => {
        ...
        // Use same offset pattern for both paths
        const sideOffset = isLeft ? -12 : 12;
        const heightOffset = isLeft ? 5 : -5;
        
        // Calculate position offset based on walking direction
        const xOffset = sideOffset;
