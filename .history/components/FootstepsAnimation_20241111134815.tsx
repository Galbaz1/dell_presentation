...
interface FootstepsAnimationProps {
  onReachFirstBlock?: () => void;
  onReachSecondBlock?: () => void;
  onReachThirdBlock?: () => void;
}

const FootstepsAnimation: React.FC<FootstepsAnimationProps> = ({
  onReachFirstBlock,
  onReachSecondBlock,
  onReachThirdBlock,
}) => {
  ...
  useEffect(() => {
    ...
    // Circle positions for triggering block reveals
    const circlePositions = {
      first: 280,   // x-coordinate of first circle
      second: 776,  // x-coordinate of second circle
      third: 1272,  // x-coordinate of third circle
    };

    const checkPosition = (x: number) => {
      if (x >= circlePositions.first && onReachFirstBlock) {
        onReachFirstBlock();
        onReachFirstBlock = undefined; // Prevent multiple triggers
      }
      if (x >= circlePositions.second && onReachSecondBlock) {
        onReachSecondBlock();
        onReachSecondBlock = undefined;
      }
      if (x >= circlePositions.third && onReachThirdBlock) {
        onReachThirdBlock();
        onReachThirdBlock = undefined;
      }
    };
  ...
  }, [onReachFirstBlock, onReachSecondBlock, onReachThirdBlock]);
