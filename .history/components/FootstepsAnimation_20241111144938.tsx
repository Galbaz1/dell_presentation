...
const FootstepsAnimation: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Add extra delay for right foot to wait for left foot during transition
    const transitionDelay = 0.7; // One step duration
    
    // Left foot starts vertical path immediately after horizontal
    animateFootstep(leftFootRef.current, verticalPath, horizontalDuration, false, true);
    // Right foot waits for an extra step duration before starting vertical path
    animateFootstep(rightFootRef.current, verticalPath, horizontalDuration + transitionDelay, true, true);
