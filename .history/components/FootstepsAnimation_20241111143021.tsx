...
const FootstepsAnimation: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Refined vertical path with subtle zigzag
    const verticalPath = [
      { x: 1520, y: 100 },   // Start where horizontal path ended
      { x: 1518, y: 150 },   // Tiny shift left
      { x: 1522, y: 200 },   // Tiny shift right
      { x: 1518, y: 250 },   // Tiny shift left
      { x: 1522, y: 300 },   // Tiny shift right
      { x: 1518, y: 350 },   // Tiny shift left
      { x: 1522, y: 400 },   // Tiny shift right
      { x: 1518, y: 450 },   // Tiny shift left
      { x: 1522, y: 500 },   // Tiny shift right
      { x: 1518, y: 550 },   // Tiny shift left
      { x: 1520, y: 600 }    // End centered
      { x: 1515, y: 150 },   // Slight left
      { x: 1525, y: 200 },   // Slight right
      { x: 1510, y: 250 },   // Slight left
      { x: 1520, y: 300 },   // Back to center
      { x: 1515, y: 350 },   // Slight left
      { x: 1525, y: 400 },   // Slight right
      { x: 1510, y: 450 },   // Slight left
      { x: 1520, y: 500 },   // Back to center
      { x: 1515, y: 550 },   // Slight left
      { x: 1525, y: 600 }    // End with slight right
    ];
    ...
    const animateFootstep = (
      ...
      path.forEach((point, index) => {
        ...
        // Adjusted offsets for more natural positioning
        const verticalOffset = isVertical 
          ? (isLeft ? 5 : -5)   // Reduced offset for vertical path
        ...
        // Simplified rotation for vertical path
        const finalRotation = isVertical 
          ? 90 + (isLeft ? -10 : 10)  // Point downward with slight angle
        ...
        gsap.to(foot, {
          ...
          duration: 0.6,
          delay: delay + (index * (isVertical ? 0.9 : 0.7)), // Slower vertical movement
          ease: "power2.inOut",
