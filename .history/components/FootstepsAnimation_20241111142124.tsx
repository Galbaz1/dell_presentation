  ...
  useEffect(() => {
    ...
    const createGhostTrail = (
      ...
      const ghostRotation = isVertical 
        ? rotation + 270  // Keep vertical rotation the same
        : rotation;       // Match the footstep rotation exactly for horizontal path
    ...
    const animateFootstep = (
      ...
      path.forEach((point, index) => {
        ...
        // Adjust vertical offset direction for downward movement
        const verticalOffset = isVertical 
          ? (isLeft ? 12 : -12)  // Horizontal offset when walking vertically
          : (isLeft ? 5 : -5);   // Vertical offset when walking horizontally
        ...
        // Add 90 degrees to rotation for vertical path
        const finalRotation = isVertical 
          ? rotation + 180 + (isLeft ? -15 : 15)  // Walking downward
          : rotation + 90 + (isLeft ? -15 : 15);  // Walking horizontally
    ...
    // Then animate vertical path with appropriate delay
    const horizontalDuration = horizontalPath.length * 0.7; // Calculate duration of horizontal path
    animateFootstep(leftFootRef.current, verticalPath, horizontalDuration, true, true);
    animateFootstep(rightFootRef.current, verticalPath, horizontalDuration + 0.35, false, true);
